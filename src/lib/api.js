...
class APIClient {
  ...
  // Centralised network-error handler. If fetch itself throws (DNS miss,
  // connection refused, TLS error) it almost always means the backend is
  // asleep — send the user to /wake which calls the waker Lambda and
  // polls /health until the backend is alive. The `?next=` param
  // preserves the path they were trying to reach so we can resume there.
  _redirectToWake() {
  ...
  async request(method, path, body, isRetry = false) {
    ...
    try {
      ...
      // /wake; throwing here is a fallback in case the navigation can't run.
  ...
  async tryRefresh() {
    try {
      ...
      // Network error here = backend asleep. Don't redirect from refresh
      // itself; let the original request() retry path catch it next.
      return false;
    }
  }
